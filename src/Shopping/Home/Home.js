...
export default function Products() {
  ...
  useEffect(() => {
    ...
    sortBy
      ? Get(
          `/shop/products?&limit=${itemPerPage}&page=${pageNum}&sortBy=${sortBy}`
        )
          .then((response) => {
    ...
    searchByName &&
      Get(
        `/shop/products?&limit=${itemPerPage}&page=${pageNum}&name=${searchByName}`
      )
        .then((response) => {
          console.log(response);
          setProducts(response.data.results);
  ...
  const state =
    ...
    JSON.stringify(localStorage.getItem("store")); //it will give us state in which array of selected products
    ...
    <div className="productImages-container">
      ...
      <div className="d-flex align-items-center">
        {pageNum !== 1 ? (
          <div onClick={() => setPageNum((prev) => prev - 1)}>
            <BsArrowLeftCircleFill size={40} style={{ marginLeft: "30%" }} />
          </div>
        ) : (
          ""
        )}
                ...
                <div key={i} className="productCard">
                  ...
                  <div className="position-relative">
                    ...
                    <div className="position-absolute top-0 end-0">
                      {state.FavoriteReducer.favorite.find(
                  ...
                  <div className="d-flex flex-column align-items-center">
                    <h6 className="py-1 mb-0 fw-bolder">
                      {item.name.length > 15
                  ...
                  <div className="mb-0 d-flex justify-content-center">
                    ...
                    {state.cartReducer.cart.find(
                      (prev) => prev._id === item._id
                    ) ? (
                      <button
                        onClick={() => navigate(`/cart?id=${item._id}`)}
      ...
      <div className="d-flex justify-content-between">
        ...
        <div className="d-flex py-1">
          ...
          <p className="px-1 fw-bolder">{itemPerPage * (pageNum - 1) + 1}</p>
          to{" "}
          <p className="px-1 fw-bolder">
            {itemPerPage * pageNum > totalResults
              ? totalResults
              : itemPerPage * pageNum}
          </p>
      ...
      {data ? (
        <QuickView show={quickView} setShow={setQuickView} data={data} />
      ) : (
        ""
      )}
    </div>
